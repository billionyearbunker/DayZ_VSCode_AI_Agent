# DayZ Custom Basebuilding Stats

## Important Notes on Data
- Name: The common display name of the tool or material as seen in the player's inventory.
- Class Name: The internal variable name used in the game configuration to spawn or script interactions for this item.
- Weight: The mass of the individual item or full stack, which impacts the player's stamina and sprint speed.
- Size: The inventory grid dimensions (Width x Height) or specific equipment slot (e.g., Shoulder) required to carry the item.
- Location: The specific map zones or building types (e.g., Industrial, Farm) where this item is most likely to spawn.
- Damage (HP/Shock): A dual value representing the lethal health damage and the non-lethal concussion damage inflicted when using this tool as a melee weapon.
- Health Used: The amount of durability points deducted from the item's total condition for every single use (e.g., one swing at a tree).
- Used For: The primary actions, crafting recipes, or construction mechanics that require this specific item.
- Stack: The maximum number of identical items that can be combined into a single inventory slot to save space.
- Description: A summary of the item's function and any visual characteristics relevant to identifying it.
...
Items that must be placed on the ground to start construction.
