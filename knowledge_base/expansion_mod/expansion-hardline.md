...
The **Expansion Hardline Mod** adds some minor features like item rarity and a player reputation system. The goal of the Hardline mod is to provide features that add endgame content and progression by using reputation points to restrict certain other Expansion mod systems and features so players can unlock them. If combined with the **DayZ-Expansion-AI** mod then the mod also allows faction persistence and players can gain reputation points for each individual valid `eAIFaction`.
